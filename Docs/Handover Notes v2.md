...
2. **E2E verification is done.** `Docs/Creeping Borders E2E Verification.md` covers all 17 patch targets + every method, checked against real decompiled vanilla bodies (vanilla code. The decompiled vanilla reference corpus was removed from the repo (kept locally only).). Verdict: PASS. Two design-intent notes in §6 (cohesion clamp saturation at high base values; double-transfer guard on occupation+policy both being active) — neither is a bug.
...
- `TI Decompiled/` — decompiled vanilla reference corpus used for E2E verification (no longer in the repo; kept locally, see BUILD NOTES.md).
